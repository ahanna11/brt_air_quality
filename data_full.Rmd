...
nearest_weather_Station <- read_csv("brt_air_quality/nearest_weather_Station.csv")
...
#Create log ozone y variable
y <- ozone_data$mean_ozone
...
new_ozone_data <- ozone_data %>%
  mutate(log_ozone = log(mean_ozone),
...
model_sp <- lm(y ~ after_wc + in_sp + after_wc_did + max_temp + min_temp + avg_temp + mean_humidity + mean_windspeed, data = combined_data)
stargazer(model_sp)
```
